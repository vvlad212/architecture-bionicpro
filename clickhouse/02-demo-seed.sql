INSERT INTO bionicpro.crm_customers_stage
    (customer_id, customer_name, customer_email, country_code, prosthesis_ids)
VALUES
    (1, 'Pilot One', 'prothetic1@example.com', 'RU', ['arm-001']),
    (2, 'Pilot Two', 'prothetic2@example.com', 'RU', ['hand-002']),
    (3, 'Pilot Three', 'prothetic3@example.com', 'DE', ['leg-003']);

INSERT INTO bionicpro.emg_sensor_data
    (customer_id, prosthesis_id, prosthesis_type, muscle_group,
     signal_frequency, signal_duration, signal_amplitude, signal_time)
VALUES
    (1, 'arm-001', 'arm', 'biceps', 180, 950, 2.450, now() - INTERVAL 47 HOUR),
    (1, 'arm-001', 'arm', 'triceps', 205, 870, 2.810, now() - INTERVAL 30 HOUR),
    (1, 'arm-001', 'arm', 'biceps', 198, 910, 2.670, now() - INTERVAL 20 HOUR),
    (2, 'hand-002', 'hand', 'forearm', 220, 740, 3.110, now() - INTERVAL 44 HOUR),
    (2, 'hand-002', 'hand', 'forearm', 215, 760, 3.050, now() - INTERVAL 18 HOUR),
    (3, 'leg-003', 'leg', 'quadriceps', 160, 1100, 2.220, now() - INTERVAL 28 HOUR);
...
INSERT INTO bionicpro.reporting_state
    (pipeline, processed_from, processed_through)
VALUES
    ('bionicpro_reporting', toStartOfDay(now() - INTERVAL 2 DAY),
     toStartOfDay(now()));
