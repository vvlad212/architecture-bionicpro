...
- API проверяет каждый день выбранного периода по журналу `reporting_processed_days`. Если хотя бы один день ещё не обработан Airflow, возвращается `409 Conflict` вместо неполного отчёта.
...
4. Запросите дату, которой нет в журнале обработанных дней, — UI покажет, что Airflow ещё не подготовил период.
5. В Airflow откройте DAG `bionicpro_reporting`; расписание — ежедневно в `00:00 UTC`. DAG также можно запустить вручную.

Demo seed заранее готовит два прошедших дня, чтобы интерфейс можно было проверить до первого планового запуска DAG.
