...
Последний шаг DAG записывает успешно обработанный день в `reporting_processed_days` и обновляет общий watermark в `reporting_state`. API сравнивает конец запрошенного диапазона с `processed_through`: более свежий период возвращает `409`, а календарные дни без измерений не блокируют выдачу уже подготовленных строк витрины.
